	...
	MongoClient.connect('mongodb://localhost:27017/test', function(err, db) {
		if (err) {
			...
			if (db) {
				...
				var page = Math.floor(Math.random() * (100000 - 1 + 1)) + 1;
					...
					if (err) {
						...
						posts.update(cur_author, {
							...
							if (err) {
								...
								//console.log("\n");
								console.log(page+','+startFind+','+endFind+','+(endFind-startFind)+','+startUpdate+','+endUpdate+','+(endUpdate-startUpdate));
								db.close();
							}
						});
					}
...
console.log("For tabulation purposes, the time values are in miliseconds");
console.log("Random,StartTimeFind,EndTimeFind,DiffTimeFind,StartTimeUpdate,EndTimeUpdate,DiffTimeUpdate");
